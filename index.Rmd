...
For some of you who are ts practitioners, there's no more coercion b/t data frame
and a time series object. For those who are new to time series, more intuitively,
fun and easier
...
Tsibble works nicely with dplyr and tidyr verbs. A new adjective you will use 
...
not only does iteration over each element, but also needs to roll. What I'm going
to do is to wake up this cat, and let her roll.
...
they are going to return a list. And in the recent version, I've added the
...
We'll see the forecasts more clearly with plots.
...
all tend to give underestimated predications. Looks like it's another hot day in January.
...
No additional steps needed. Just as what we did before, we can directly pipe 
them into `model()`, and it will fit a model for each time series at once. 
